...
function splitGuard(summary: string) {
  // if guard tags exist, keep them visible but separate from the “opinion”
  const lines = summary.split("\n");
  const guardLines = lines.filter((l) => l.includes("[CITATION_GUARD]") || l.includes("[CITATION_DB_GUARD]"));
...
export function VerdictCard({
  ...
  const { guard, body } = splitGuard(verdict.analysis.summary || "");
  const citations = uniq((verdict.relevant_citations || []).filter(Boolean));

  const strength = (verdict.analysis.strength_factors || []).filter(Boolean);
  const weakness = (verdict.analysis.weakness_factors || []).filter(Boolean);
  const missing = (verdict.analysis.missing_elements || []).filter(Boolean);
  const findings = (verdict.analysis.findings || []).filter(Boolean);
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${statusTone(verdict.status)}`}>
              {verdict.status}
            </span>
            <span className="text-xs text-white/60">
              confidence: <span className="font-semibold text-white/80">{pct(verdict.confidence_score)}</span>
            </span>
          </div>
        </div>

        {hasGuardText(verdict.analysis.summary || "") && (
          <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
            ...
            <div className="mt-1 whitespace-pre-wrap text-amber-100/90">{guard || "Some citations were filtered/removed."}</div>
          </div>
        )}
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 whitespace-pre-wrap text-sm text-white/85">{body || verdict.analysis.summary}</div>
      </div>
                    ...
                    <div>
                      ...
                      <div className="mt-1 space-y-2">
                        {(f.evidence_quotes || []).map((q, qi) => (
                          <blockquote key={qi} className="rounded-lg border border-white/10 bg-black/20 p-2">
...
function Section({
  ...
  const isEmpty =
    ...
    (childArray.length === 1 &&
      ...
      // BulletList returns <div> with “empty” text if items empty — so just render children always
      false);

  return (
    <div className="rounded-xl border border-white/10 bg-black/10 p-4">
      <div className="text-xs font-semibold text-white/70">{title}</div>
      <div className="mt-2">{isEmpty ? <div className="text-sm text-white/60">{emptyText}</div> : children}</div>
    </div>
  );
}
